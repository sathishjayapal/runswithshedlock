...
public class RunEventAssembler implements SimpleRepresentationModelAssembler<RunEventDTO> {
    ...
    public void addLinks(final EntityModel<RunEventDTO> entityModel) {
        entityModel.add(linkTo(methodOn(RunEventResource.class).getRunEvent(entityModel.getContent().getId())).withSelfRel());
        entityModel.add(linkTo(methodOn(RunEventResource.class).getAllRunEvents(null, null)).withRel(IanaLinkRelations.COLLECTION));
    }

    @Override
    public void addLinks(final CollectionModel<EntityModel<RunEventDTO>> collectionModel) {
        collectionModel.add(linkTo(methodOn(RunEventResource.class).getAllRunEvents(null, null)).withSelfRel());
    }
