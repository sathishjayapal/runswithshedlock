...
public class GarminRunAssembler implements SimpleRepresentationModelAssembler<GarminRunDTO> {
    ...
    public void addLinks(final EntityModel<GarminRunDTO> entityModel) {
        entityModel.add(linkTo(methodOn(GarminRunResource.class)
                        .getGarminRun(entityModel.getContent().getId()))
                .withSelfRel());
        entityModel.add(linkTo(methodOn(GarminRunResource.class).getAllGarminRuns(null, null))
                .withRel(IanaLinkRelations.COLLECTION));
        if (entityModel.getContent().getRunner() != null) {
            entityModel.add(linkTo(methodOn(RunnerResource.class)
                            .getRunner(entityModel.getContent().getRunner()))
                    .withRel("runner"));
        }
    }
    ...
    public void addLinks(final CollectionModel<EntityModel<GarminRunDTO>> collectionModel) {
        collectionModel.add(linkTo(methodOn(GarminRunResource.class).getAllGarminRuns(null, null))
                .withSelfRel());
    }
    ...
    public EntityModel<SimpleValue<Long>> toSimpleModel(final Long id) {
        ...
        simpleModel.add(
                linkTo(methodOn(GarminRunResource.class).getGarminRun(id)).withSelfRel());
        return simpleModel;
    }
}
